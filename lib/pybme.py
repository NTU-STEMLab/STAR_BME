...
@author: KSJ
@STEMLAB, Department of Bioenvironmental Systems Engineering,
...
from lib.rawdata2griddata import rawdata2griddata, rawdata2griddataforcoor
from lib.rawdata2griddata import rawdata2griddata_list
from lib.findzfromgriddata import findzfromgriddata
from .starpy.starpy.stats.stcov import stcov_split as stcov
from lib.stcovfit import fitcovariance, estimatecovariance
from .starpy.starpy.bme.BMEoptions import BMEoptions as bmeoptions
from .starpy.starpy.bme.softconverter import gs2ud, uf2ud, ud2ud, ud2zs_temp 
from .starpy.starpy.bme.softconverter import proba2stat, proba2probdens
from .starpy.starpy.bme.softconverter import proba2quantile
from lib.nousedataobj import NoUseDataObj
from .starpy.starpy.bme.BMEprobaEstimations import BMEPosteriorMoments
from lib.BMEprobaMoments import BMEprobaMoments


class GuiManager(object):
    '''處理gui相關的事，讓此模組在有無gui介面下皆可運行'''
    def setProgressDialog(self, qpgd, process_events = None):
...
class SaveLoadManager(object):
    ...
    def save( self, pickler ):
        #dump info
        pickler.dump( '{i},{n}'.format(
            i = self.name, n = len(self.save_list))
        )
...
class SoftData(CoordinateData):
    ...
    def setValueFromFile(self, file_name, usecols = (3,4), skiprows = 0,
                         delimiter = ",", count_per_read = 2000):
        if self.input_format.lower() in ("gaussian", "uniform", "linear"):
        ...
        softpdftype = self.pdf_type  # use actual soft PDF type instead of 'dummy'
        if self.input_format.lower() == "user defined": # user defined
            ...
            self.nl, self.limi, self.probdens, _ = proba2probdens( softpdftype, self.nl, self.limi, self.probdens ) #normalize
            self.quantile = proba2quantile( softpdftype, self.nl, self.limi, self.probdens )
            self.z, self.var = proba2stat( softpdftype, self.nl, self.limi, self.probdens )
...
class EstimatedData(CoordinateData):
    ...
    def updateDataInfo(self): #overload CoordnaiteData
        ...
        try:
            # FIX D (part 1): Guard against negative variances from noisy
            # QMC integration — clamp to zero before sqrt.
            _safe_v = numpy.maximum(zwithoutnan_v, 0.0)
            _lo = zwithoutnan_m - 1.96 * numpy.sqrt(_safe_v)
            _hi = zwithoutnan_m + 1.96 * numpy.sqrt(_safe_v)
            # Drop any NaN/Inf that survived
            _lo = _lo[numpy.isfinite(_lo)]
            _hi = _hi[numpy.isfinite(_hi)]
            if _lo.size == 0 or _hi.size == 0:
                raise ValueError("no finite bounds")
            self.z_all_min_without_nan = _lo.min()
            self.z_all_max_without_nan = _hi.max()
        except ValueError:
...
class Covariance(SaveLoadManager, GuiManager):
    ...
    def calculate(self):
        ...
        parameters = (self.spatial_lags[0], self.spatial_tolerant_ranges[0],
                      ...
                      None,None, # ang, angtol
                      self)
        needed_param = needed_data + parameters 

        #self.stvn = stcov(*needed_param)
        self.vnst = stcov(*needed_param)

        if not self.vnst: # user cancel
            return self.vnst
        else:
            self.stvn = (self.vnst[2], self.vnst[3],
                self.vnst[0], self.vnst[1])
    ...
    def selectParamByBobyqa( self, init_guess, args, low_bnd, up_bnd ):
        ...
        from .starpy.starpy.general.bobyqa import bobyqa
...
class BmeObj(SaveLoadManager, GuiManager):
    def __init__(self, h_data=None, s_data=None, e_data=None,
        trend=None, covariance=None):
        ...
        self.order = 'Zero Mean'
        # 'Zero Mean' (QGIS), numpy.nan (Py) means Zero Mean (Default)
    ...
    def detrend(self,method = "No Detrending", parameters = ()):
        ...
        if data: #tuple or False
            ...
            return False
        trend_obj.setCoordinate(coor_tuple=())
        #determind detrend, not estimate trend
        ...
        #
        if not self._setDataTrend(
            (trend_obj.data[0],trend_obj.data[1],tr_grid)
            ):
            return False
        else: 
    ...
    def getFunctionParameters(self):
        ...
        def getCovModel():
            def convertPythonModelName(modelname): 
                #convertPyModelNameToCModelIndex
                #                     C   PYTHON
                #//exponentialC     = 1     1
                #//gaussianC        = 2     0
                #//holesinC         = 3     
            ...
            for bv in self.covariance.models:
                ...
                covmodel.append(
                    convertPythonModelName(bv[1])
                    +'/'
                    +convertPythonModelName(bv[3])
                    )
        ...
        if self.hard_data.hasData():
            ...
            ch = None #numpy.array([],ndmin=2).reshape((0, ck.shape[1]))
            zh = None #numpy.array([],ndmin=2).reshape((0, 1))
        ...
        if self.soft_data.hasData():
            ...
            nlseq, limiseq, probadensseq, _ = proba2probdens(
                softpdftype,
                self.soft_data.nl,
            ...
            cs = None #numpy.array([],ndmin=2).reshape((0, ck.shape[1]))
        ...
        nhmax, nsmax, dmax, order, options =\
            self.nhmax, self.nsmax, self.dmax, self.order, self.options

        return [ck, ch, cs, zh, zs,
                covmodel, covparam,
                nhmax, nsmax, dmax,
                order, options]
    ...
    def estimate(self, n_workers=1):
        (ck, ch, cs, zh, zs, covmodel, covparam, nhmax, nsmax, dmax,
        ...
        zk = BMEPosteriorMoments(
            ck, ch, cs, zh, zs, covmodel, covparam, None,
            order, options, nhmax, nsmax, dmax,
            gui_args=(self.progress_dialog,),
            n_workers=n_workers)

        if isinstance(zk, numpy.ndarray):
            moments_temp = zk
            info_temp = numpy.empty(zk.shape)
        else:
        ...
        self.estimated_data.z_mean = (
            moments_np[:,0]
            + self.estimated_data.tr.flatten()).reshape((-1,1)
            )
        self.estimated_data.z_variance = moments_np[:,1].reshape((-1,1))
        
        mean_grid = rawdata2griddata(
            self.estimated_data.x,
        ...
        variance_grid = rawdata2griddata(
            self.estimated_data.x,
            self.estimated_data.y,
            self.estimated_data.t,
    ...
    def _getGridData(self, flag):
        ...
        #flag can be "z" "tr"
        if self.hard_data.hasData() and self.soft_data.hasData():
            #has hard and soft
            x_all, y_all, t_all, flag_all =\
                list(map(
                    numpy.vstack,
                    (
                        ...
                        (getattr(self.hard_data, flag),
                            getattr(self.soft_data, flag))
                    )
                ))
    ...
    def _setDataTrend(self,data):
        def byDataClass(dataobj):
            ...
            needed_data = (dataobj.x, dataobj.y, dataobj.t,
                           tr_temp, self)
            tr_grid_temp = rawdata2griddata(*needed_data)
            # (s_grid, t_grid, z_grid)
    ...
    def setDmax(self, dmax):
        dmax = numpy.array(dmax, dtype=numpy.float64, ndmin=2)
        if dmax.shape[1] !=3: # column not match
            return False
    ...
    def crossValidation(
        self, sample_number = None, boundary = None, use_data = 'hard',
        ...
        #combine
        if self.hard_data.hasData():
            hard_dataset = numpy.hstack(
                (
                    self.hard_data.x, self.hard_data.y,
                    self.hard_data.t, self.hard_data.z,
                    self.hard_data.tr
                )
            )
        if self.soft_data.hasData():
            soft_dataset = numpy.hstack(
                (
                    self.soft_data.x, self.soft_data.y,
                    self.soft_data.t, self.soft_data.z,
                    self.soft_data.tr
                )
            )
        ...
        if not boundary:
            ...
            if use_data in ['hard','both']:
                condition_x = ( hard_dataset[:,0] >= xmin )\
                    & ( hard_dataset[:,0] <= xmax )
                condition_y = ( hard_dataset[:,1] >= ymin )\
                    & ( hard_dataset[:,1] <= ymax )
                condition_t = ( hard_dataset[:,2] >= tmin )\
                    & ( hard_dataset[:,2] <= tmax )
                hard_original_index = numpy.where(
                    condition_x & condition_y & condition_t
                    )[0]
                clip_hard_dataset = hard_dataset[
                    condition_x & condition_y & condition_t
                    ]
            ...
            if use_data in ['soft','both']:
                condition_x = ( soft_dataset[:,0] >= xmin )\
                    & ( soft_dataset[:,0] <= xmax )
                condition_y = ( soft_dataset[:,1] >= ymin )\
                    & ( soft_dataset[:,1] <= ymax )
                condition_t = ( soft_dataset[:,2] >= tmin )\
                    & ( soft_dataset[:,2] <= tmax )
                soft_original_index = numpy.where(
                    condition_x & condition_y & condition_t )[0]
                clip_soft_dataset = soft_dataset[
                    condition_x & condition_y & condition_t
                    ]
        ...
        all_sample_index = numpy.array(
            sorted(
                random.sample(range(sample_number_all ), sample_number)
                )
            )
        #change to original index
        hard_sample_index =\
            all_sample_index[
                all_sample_index <= len_clip_hard_dataset - 1 
                ]
        soft_sample_index =\
            all_sample_index[
                all_sample_index > len_clip_hard_dataset - 1
                ] - len_clip_hard_dataset
        ...
        # Helper to keep UI alive during long loops
        ...
        # -- Trend estimation with progress updates --
        _trend_t0 = time.time()

        # ====================================================================
        # OPTIMISATION: For "No Detrending" the trend is always zero.
        # Skip the extremely expensive leave-one-out loop that calls
        # rawdata2griddata() N times (O(N²) total) and just fill zeros.
        # ====================================================================
        if detrend_method.lower() in ("no detrending", "nd"):
            # Trend is zero for every sample point
            ...
            if len(soft_sample_index):
                for idx in soft_original_index[soft_sample_index]:
                    tr_cv_s[idx][0] = 0.0
            print("[STAR_BME CV] No detrending — trend set to zero "
                  "(skipped expensive LOO loop) in %.3fs"
                  % (time.time() - _trend_t0))

        else:
            # Full leave-one-out trend estimation (needed for KS, STMean, etc.)
            _n_trend_total = len(hard_sample_index) + len(soft_sample_index)
            self.setProgressRange(0, _n_trend_total)
            self.setCurrentProgress(
                0, "Estimating trend (0/%d)..." % _n_trend_total)
            _trend_count = 0
            ...
            if len(hard_sample_index):
                for idx in hard_original_index[hard_sample_index]:
                    dataset = \
                        all_dataset[numpy.arange(len(all_dataset)) != idx, :]
                    x, y, t, z, dummy_tr = \
                        list(map(
                            lambda x: numpy.array(x, ndmin=2).T,
                    ...
                    if result:
                        ...
                        _est_val = tr_obj.estimate()
                        tr_cv_h[idx][0] = numpy.asarray(_est_val).flat[0]
                    else:
                        return False

                    _trend_count += 1
                    if (_trend_count % max(1, _n_trend_total // 20) == 0
                            or _trend_count == _n_trend_total):
                        _trend_elapsed = time.time() - _trend_t0
                        self.setCurrentProgress(
                            _trend_count,
                            "Estimating trend (%d/%d, %.0fs)..."
                            % (_trend_count, _n_trend_total,
                               _trend_elapsed))
                        _process_qt_events()
            ...
            if len(soft_sample_index):
                for idx in soft_original_index[soft_sample_index]:
                    if use_data == 'both':
                        ...
                        idx2 = idx
                    dataset = all_dataset[
                        numpy.arange(len(all_dataset)) != idx2, :
                        ]
                    x, y, t, z, dummy_tr = \
                        list(map(
                            lambda x: numpy.array(x, ndmin=2).T,
                    ...
                    if result:
                        grid_s, grid_t, grid_z = result
                        tr_obj.setData(result)
                        x_est, y_est, t_est = list(map(
                             lambda x: numpy.array([x], ndmin=2).T,
                             all_dataset[idx2, :3]
                             ))
                        tr_obj.setCoordinate((x_est, y_est, t_est))
                        _est_val = tr_obj.estimate()
                        tr_cv_s[idx][0] = numpy.asarray(_est_val).flat[0]
                    else:
                        return False
            ...
            print("[STAR_BME CV] Trend estimation done in %.1fs"
                  % (time.time() - _trend_t0))
        ...
        #set progressbar — show immediately (default minimumDuration=4s)
        ...
        # ---- Build work items for all CV points ----
        self.setCurrentProgress(0, "Preparing CV work items...")
        _process_qt_events()
        _build_t0 = time.time()
        _cv_work_items = []  # list of dicts with BMEprobaMoments args

        # Pre-compute proba2probdens for ALL soft data ONCE (shared by
        # hard-point and soft-point CV iterations).  Previously this was
        # called inside the soft-data loop, making work-item building
        # O(n_soft²).  Since proba2probdens normalises each row
        # independently, we pre-compute once and slice per iteration.
        if self.soft_data.hasData():
            _precomp_nlseq, _precomp_limiseq, _precomp_probadensseq, _ = \
                proba2probdens(
                    self.soft_data.pdf_type,
                    self.soft_data.nl,
                    self.soft_data.limi - self.soft_data.tr,
                    self.soft_data.probdens
                )
        else:
            _precomp_nlseq = numpy.array([], ndmin=2)
            _precomp_limiseq = numpy.array([], ndmin=2)
            _precomp_probadensseq = numpy.array([], ndmin=2)

        # ============================================================
        # Build shared data dict + lightweight work items
        # ============================================================
        # ThreadPoolExecutor: shared_dict lives in the same process
        # — zero copy overhead, threads read directly from memory.
        # Each work item carries only a data_type + index.
        # ============================================================
        covmodel_cv, covparam_cv = getCovModel()

        # Pre-compute zs in the format expected by BMEPosteriorMoments:
        # a list of [softpdftype, nl_i, limi_i, probdens_i] per soft datum.
        if self.soft_data.hasData():
            _precomp_zs = ud2zs_temp(
                self.soft_data.pdf_type,
                _precomp_nlseq, _precomp_limiseq, _precomp_probadensseq)
        else:
            _precomp_zs = []

        _shared_dict = {
            'hard_coords': (hard_dataset[:, :3].copy()
                            if self.hard_data.hasData()
                            else numpy.array([], ndmin=2)),
            'hard_zh': ((hard_dataset[:, 3:4] - hard_dataset[:, 4:5]).copy()
                        if self.hard_data.hasData()
                        else numpy.array([], ndmin=2)),
            'soft_coords': (soft_dataset[:, :3].copy()
                            if self.soft_data.hasData()
                            else numpy.array([], ndmin=2)),
            'zs':          _precomp_zs,
            'covmodel':    covmodel_cv,
            'covparam':    covparam_cv,
            'nhmax':       self.nhmax,
            'nsmax':       self.nsmax,
            'dmax':        self.dmax,
            'order':       self.order,
        ...
        # Work items are intentionally tiny: just a type tag and an
        # index.  The worker reconstructs leave-one-out arrays from
        # _shared_dict, keeping IPC overhead negligible.
        if len(hard_sample_index):
            for idx in hard_original_index[hard_sample_index]:
                _cv_work_items.append({
                    'data_type': 'hard', 'idx': int(idx)})

        if len(soft_sample_index):
            for idx in soft_original_index[soft_sample_index]:
                _cv_work_items.append({
                    'data_type': 'soft', 'idx': int(idx)})

        print("[STAR_BME CV] Work items built in %.1fs (%d items)"
              % (time.time() - _build_t0, len(_cv_work_items)))

        self.setCurrentProgress(
            0, "Starting BME estimation (%d points)..." % len(_cv_work_items))
        _process_qt_events()

        # ====================================================================
        # Non-blocking dispatch — ThreadPoolExecutor (QGIS-safe)
        # ====================================================================
        # Why ThreadPoolExecutor and NOT ProcessPoolExecutor?
        #
        # 1. QGIS on macOS sets sys.executable to the QGIS app binary
        #    (e.g. /Applications/QGIS.app/Contents/MacOS/QGIS).
        #    multiprocessing.spawn uses sys.executable to start child
        #    processes, so spawned children launch the QGIS binary
        #    instead of Python, producing "Invalid Data Source: /-c"
        #    errors and a deadlock (parent blocks writing to pipe that
        #    no child ever reads).
        #
        # 2. The BME prediction module already uses ThreadPoolExecutor
        #    with up to 12 workers successfully.  NumPy/SciPy release
        #    the GIL during BLAS/LAPACK C routines, so threads achieve
        #    real parallelism for the linear-algebra-heavy BME work.
        #
        # Fallback chain:
        #   1. ThreadPoolExecutor(N)  — parallel, GIL-releasing LA ops
        #   2. ThreadPoolExecutor(1)  — sequential, still keeps UI alive
        # ====================================================================
        from concurrent.futures import (ThreadPoolExecutor,
                                        wait, FIRST_COMPLETED)
        from .BMEprobaMoments import (_cv_init_worker, _cv_worker_mp)

        _cancelled = False
        _n_ok = 0
        _n_fail = 0

        _actual_workers = max(1, min(n_workers, len(_cv_work_items)))

        # Limit BLAS internal threads to prevent over-subscription
        import os as _os
        _orig_blas_env = {}
        for _bvar in ('OPENBLAS_NUM_THREADS', 'MKL_NUM_THREADS',
                       'VECLIB_MAXIMUM_THREADS', 'NUMEXPR_NUM_THREADS'):
            _orig_blas_env[_bvar] = _os.environ.get(_bvar)
            _os.environ[_bvar] = '1'

        _executor = None

        # --- Initialise shared data (threads share memory) ---
        _cv_init_worker(_shared_dict)

        # --- Create ThreadPoolExecutor ---
        # No smoke test needed — ThreadPoolExecutor doesn't spawn
        # child processes, so there's no risk of QGIS-binary issues.
        try:
            _executor = ThreadPoolExecutor(max_workers=_actual_workers)
            print("[STAR_BME CV] ThreadPoolExecutor (%d threads) created"
                  % _actual_workers)
        except Exception as _e:
            print("[STAR_BME CV] ThreadPoolExecutor(%d) failed: %s "
                  "— falling back to 1 thread" % (_actual_workers, _e))
            _executor = ThreadPoolExecutor(max_workers=1)
            _actual_workers = 1

        print("[STAR_BME CV] Dispatching %d points across %d threads..."
              % (len(_cv_work_items), _actual_workers))

        self.setCurrentProgress(
            0, "Starting BME cross-validation (%d points, %d threads)..."
            % (len(_cv_work_items), _actual_workers))
        _process_qt_events()

        try:
            if len(_cv_work_items) == 0:
                print("[STAR_BME CV] No work items — nothing to do")
            else:
                # Submit ALL jobs upfront
                futures = {
                    _executor.submit(_cv_worker_mp, item): item
                    for item in _cv_work_items
                }
                pending = set(futures.keys())

                # Poll loop: wait() with short timeout lets us
                # call processEvents() regularly so the progress
                # bar repaints and the Cancel button responds.
                _POLL_SEC = 0.20  # max 200 ms between UI refreshes

                while pending:
                    done, pending = wait(
                        pending, timeout=_POLL_SEC,
                        return_when=FIRST_COMPLETED)

                    # ---- Collect completed results FIRST ----
                    for future in done:
                        try:
                            data_type, idx, value = future.result()
                        except Exception as exc:
                            print("[STAR_BME CV] Future error: %s" % exc)
                            cv_iter_count += 1
                            _n_fail += 1
                            continue
                        ...
                        cv_iter_count += 1
                        total_elapsed = time.time() - cv_t0
                        avg_per_iter = (total_elapsed
                                        / max(cv_iter_count, 1))
                        remaining = avg_per_iter * (
                            sample_number - cv_iter_count)
                        prog_text = (
                            "CV %s %d/%d  (%.1fs/pt, "
                            "elapsed %.0fs, ETA %.0fs)"
                            % (data_type, cv_iter_count,
                               sample_number, avg_per_iter,
                               total_elapsed, remaining)
                        )
                        self.setCurrentProgress(
                            cv_iter_count, prog_text)
                        # Log every 50th or first 5
                        if (cv_iter_count <= 5
                                or cv_iter_count % 50 == 0):
                            print("[STAR_BME CV] " + prog_text)
                    ...
                    # ---- Update elapsed-time status when no
                    #      futures completed (keeps user informed) --
                    if not done and cv_iter_count == 0:
                        _elapsed = time.time() - cv_t0
                        self.setCurrentProgress(
                            0,
                            "Computing first results... "
                            "(elapsed %.0fs, %d threads)"
                            % (_elapsed, _actual_workers))

                    # ---- Flush Qt events AFTER progress update ----
                    # Critical: processEvents() must run AFTER
                    # setCurrentProgress() so the new label text
                    # and bar position are rendered immediately.
                    _process_qt_events()
            ...
            # Always clean up executor and restore BLAS env vars
            if _executor is not None:
                try:
                    _executor.shutdown(wait=False, cancel_futures=True)
                except TypeError:
                    _executor.shutdown(wait=False)
            for _bvar, _bval in _orig_blas_env.items():
                if _bval is None:
                    _os.environ.pop(_bvar, None)
                else:
                    _os.environ[_bvar] = _bval

        print("[STAR_BME CV] Dispatch finished: ok=%d, fail=%d"
              % (_n_ok, _n_fail))

        if _cancelled:
            return False

        total_cv_time = time.time() - cv_t0
        print("[STAR_BME CV] Cross-validation complete: %d iterations "
              "in %.1fs (avg %.2fs/iter)"
              % (cv_iter_count, total_cv_time,
                 total_cv_time / max(cv_iter_count, 1)))

        # --- Diagnostic summary ---
        n_h_valid = int(numpy.count_nonzero(~numpy.isnan(z_cv_h))) if z_cv_h.size > 0 else 0
        n_s_valid = int(numpy.count_nonzero(~numpy.isnan(z_cv_s))) if z_cv_s.size > 0 else 0
        print("[STAR_BME CV] Results: hard valid=%d/%d, soft valid=%d/%d"
              % (n_h_valid, z_cv_h.size, n_s_valid, z_cv_s.size))


        #get z_cv_grid tr_cv_grid from hard data
        if self.hard_data.hasData():
            result = rawdata2griddata(
                self.hard_data.x, self.hard_data.y, self.hard_data.t,
                tr_cv_h, self
                )
            if result:
        ...
        if self.soft_data.hasData():
            ...
            result = rawdata2griddata(
                self.soft_data.x, self.soft_data.y, self.soft_data.t,
                tr_cv_s, self
                )
            if result:
        ...
        # Mean, Standard Deviation, Median, Min Value, Max Value
        # Only include data types that were actually cross-validated
        _error_parts = []
        if use_data in ('hard', 'both') and self.hard_data.hasData():
            error_h = self.hard_data.z - self.hard_data.z_cv
            error_h_without_nan = error_h[~numpy.isnan(error_h)]  # flat to 1D
            _error_parts.append(error_h_without_nan)
            _n_valid_h = len(error_h_without_nan)
        else:
            _n_valid_h = 0
        if use_data in ('soft', 'both') and self.soft_data.hasData():
            error_s = self.soft_data.z - self.soft_data.z_cv
            error_s_without_nan = error_s[~numpy.isnan(error_s)]
            _error_parts.append(error_s_without_nan)
            _n_valid_s = len(error_s_without_nan)
        else:
            _n_valid_s = 0

        if _error_parts:
            error_all = numpy.hstack(_error_parts)
        else:
            error_all = numpy.array([])

        _n_valid = len(error_all)
        _n_total = sample_number
        print("[STAR_BME CV] Error statistics: %d/%d valid results "
              "(hard=%d, soft=%d)" % (_n_valid, _n_total,
                                      _n_valid_h, _n_valid_s))

        if _n_valid == 0:
            print("[STAR_BME CV] WARNING: All cross-validation results are "
                  "NaN — no valid error statistics can be computed. "
                  "Check the QGIS Python Console for '[STAR_BME CV] "
                  "Exception' or '[STAR_BME CV] Worker exception' "
                  "messages for details.")
            # Return NaN statistics so the caller can still display a
            # result dialog instead of crashing
            _nan = float('nan')
            return (_nan, _nan, _nan, _nan, _nan, _nan, _nan, _nan)
        ...
        return (error_all_rmse, error_all_mean, error_all_std,
                error_all_25_percentile,
                error_all_median,
                error_all_75_percentile,
...
if __name__ == "__main__": #Non-GUI test
    ...
    if bmeobj.load( r'C:\Users\scku\Desktop\bmeobj_test555.bme' ):
        pass
    bmeobj.setOrder(numpy.nan)
    bmeobj.setOptions(3,0.001)
    bmeobj.setOptions(2,100000)
    bmeobj.setNsmax(2)
    bmeobj.estimate()
    for i in zip(
        bmeobj.estimated_data.x,
        bmeobj.estimated_data.y,
        bmeobj.estimated_data.t,
        bmeobj.estimated_data.z_mean,
        bmeobj.estimated_data.z_variance):
        print(numpy.array(i).reshape(1,-1))
