    ...
    if not getattr(self, 'dependencies_ok', False):
        try:
            ...
            QMessageBox.critical(
                ...
                f"import subprocess, sys, os, platform\n"
                f"# Windows:\n"
                f"#   python_exe = os.path.join(sys.prefix, 'python3.exe')\n"
                f"# macOS / Linux:\n"
                f"#   python_exe = os.path.join(sys.prefix, 'bin', 'python3')\n"
                f"python_exe = os.path.join(sys.prefix, 'python3.exe') if platform.system() == 'Windows' else os.path.join(sys.prefix, 'bin', 'python3')\n"
